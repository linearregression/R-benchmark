## title: R benchmark
# R Benchmark HTML report (10/2013) [Renaud Gaujoux]
# The script was modified to be spinned by knitr and generate plots 
# http://github.com/renozao/R-benchmark
...
if( require(SuppDists)	){# Optimized random number generators
...
do_replicate <- local({
  ...
  function(n, expr){
    ...
    if( is.numeric(n) && missing(expr) ){
      .res <<- matrix(as.numeric(NA), 0, n)
    }else if( is.character(n) ){
      if( missing(expr) ) expr <- 1L
      mat <- matrix(as.numeric(NA), expr, ncol(.res), dimnames = list(rep(n, expr), NULL))
      .res <<- rbind(.res, mat)
    }else{
      expr <- substitute(expr)
      timing <- replicate(ncol(.res), {
            eval(expr)
          })
      .res[.i, ] <<- timing
      .i <<- .i +1
...
if( Revo <- 'Revobase' %in% loadedNamespaces() ){
  setMKLthreads(1)
}
cat("R flavour: ", ifelse(Revo, "Revolution R", "Base R"), "\n")
cat("   ===============\n")
...
do_replicate('Matrix functions', 5L)
...
remove("cumulate", "timing", "runs")
cat("                      --- End of test ---\n\n")

# save results
timing <- do_replicate()
colnames(times) <- unique(rownames(timing))
saveRDS(times, file = 'timing.rds')
saveRDS(timing, file = 'timing_all.rds')

# plot
library(ggplot2)
library(reshape2)
ggplot(melt(timing), aes(factor(Var1), value)) + geom_boxplot()
